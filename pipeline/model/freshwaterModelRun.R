...
# This script is for use in Sigma2, and runs the model. The second half creates the outputs we need for figures
...
# if (omradeNameNoDigits %in% c("Mjøsa", "Midtre Telemark")) {stop("Skipping this catchment due to known issues with the network.")}
...
# Load in metricGraph
load(paste0("data/external/riverNetwork/waterAreaGraphs/", omradeNameNoDigits,"/waterAreaGraph.RData"))
...
Likelihoods <- MakeLikelihoods(data = modelData, 
                               ...
                               predictionFormula = list(intensity = ~ PA_intercept + elvelengde + elevation_variability + summer_temperature +
...
timeTaken <- end - start
print(timeTaken)
saveRDS(4 * timeTaken, paste0(modelOutputPath, "/", omradeName, "_timeTaken.RDS"))
...
#otherIntercepts <- model$summary.fixed["PA_intercept","mean"]
